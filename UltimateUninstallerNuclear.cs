...
class UltimateUninstallerNuclear
{
    ...
    private static bool opencodeMode = false;
    private static HashSet<string> opencodeExactPaths = new HashSet<string>();
    private static List<string> opencodeScopedFolders = new List<string>();
    private static HashSet<string> opencodeHardDeleteFolders = new HashSet<string>();
    ...
    static void Main(string[] args)
    {
        ...
        InitializeTargetContext(patterns);
    ...
    static bool IsSafePath(string path)
    {
        ...
        // Exception: when targeting opencode, allow scanning/deleting inside explicit opencode scopes
        // even if path contains protected tokens like \node_modules or \npm.
        if (opencodeMode && IsInOpencodeScope(lowerPath))
            return false;
    ...
    static void InitializeTargetContext(List<string> patterns)
    {
        opencodeMode = patterns.Any(p => p.Contains("opencode"));
        opencodeExactPaths.Clear();
        opencodeScopedFolders.Clear();
        opencodeHardDeleteFolders.Clear();
        ...
        // Exact file targets from reported leftovers.
        opencodeExactPaths.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "node_modules", "opencode-ai", "node_modules", "opencode-windows-x64", "bin", "opencode.exe")));
        opencodeExactPaths.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "node_modules", "opencode-ai", "node_modules", "opencode-windows-x64-baseline", "bin", "opencode.exe")));
        opencodeExactPaths.Add(NormalizePath(Path.Combine(userProfile, ".local", "share", "opencode", "opencode.db-wal")));
        opencodeExactPaths.Add(NormalizePath(Path.Combine(userProfile, ".local", "share", "opencode", "opencode.db")));
        opencodeExactPaths.Add(NormalizePath(Path.Combine(userProfile, ".local", "share", "opencode", "opencode.db-shm")));
        opencodeExactPaths.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "node_modules", "opencode-ai", "bin", "opencode")));
        opencodeExactPaths.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "opencode.ps1")));
        opencodeExactPaths.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "opencode")));
        opencodeExactPaths.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "opencode.cmd")));

        // Scoped folders: include future-created content but still delete only opencode-matching names.
        opencodeScopedFolders.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "node_modules", "opencode-ai", "node_modules", "opencode-windows-x64-baseline")));
        opencodeScopedFolders.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "node_modules", "opencode-ai", "node_modules", "opencode-windows-x64")));
        opencodeScopedFolders.Add(NormalizePath(Path.Combine(localAppData, "npm-global", "node_modules", "opencode-ai")));
        opencodeScopedFolders.Add(NormalizePath(Path.Combine(userProfile, ".local", "state", "opencode")));
        opencodeScopedFolders.Add(NormalizePath(Path.Combine(userProfile, ".local", "share", "opencode")));
        opencodeScopedFolders.Add(NormalizePath(Path.Combine(userProfile, ".config", "opencode")));
        opencodeScopedFolders.Add(NormalizePath(Path.Combine(userProfile, ".cache", "opencode")));

        // Explicit leftover folders the user requested to remove entirely.
        foreach (var folder in opencodeScopedFolders)
            opencodeHardDeleteFolders.Add(folder);
    }
    ...
    static bool IsInOpencodeScope(string lowerPath)
    {
        if (opencodeExactPaths.Contains(lowerPath))
            return true;
    ...
    static void NukeOpencodeTargets(List<string> patterns)
    {
        if (!opencodeMode)
            return;

        Console.WriteLine("\n  [OPENCODE] Targeted residue sweep...");

        // Exact files/scripts/wrappers.
        foreach (var target in opencodeExactPaths)
        {
            try
            {
                string targetName = Path.GetFileName(target).ToLower();
                bool matches = IsExactMatch(targetName, patterns) || IsExactMatch(target, patterns);
                if (!matches) continue;

                UpdateProgress("OPENCODE: " + targetName, true);
                if (File.Exists(target))
                {
                    Console.WriteLine("  [OPENCODE FILE] " + target);
                    ForceDeleteFile(target);
                }
                else if (Directory.Exists(target))
                {
                    Console.WriteLine("  [OPENCODE DIR] " + target);
        ...
        // Scoped folder sweep: delete only opencode-matching names within the listed opencode folders.
        foreach (var scope in opencodeScopedFolders)
        {
                ...
                while (pendingDirs.Count > 0)
                {
                    ...
                    try
                    {
                        foreach (var file in Directory.GetFiles(currentDir, "*", SearchOption.TopDirectoryOnly))
                        {
                            ...
                            {
                                Console.WriteLine("  [OPENCODE MATCH FILE] " + file);
                    ...
                    try
                    {
                        foreach (var dir in Directory.GetDirectories(currentDir, "*", SearchOption.TopDirectoryOnly))
                        {
                            if (IsSafePath(dir))
                                continue;
                            ...
                            if (IsExactMatch(dirName, patterns))
                            {
                                Console.WriteLine("  [OPENCODE MATCH DIR] " + dir);
                                ForceDeleteDirectory(dir);
                            }
                            else
                            {
            ...
            // Prune empty matching directories so leftover opencode folders disappear when empty.
            try
            {
                var allDirs = Directory.GetDirectories(scope, "*", SearchOption.AllDirectories)
                ...
                foreach (var dir in allDirs)
                {
                    ...
                    bool isEmpty = !Directory.EnumerateFileSystemEntries(dir).Any();
                    if (isEmpty)
                        ForceDeleteDirectory(dir);
                }

                string rootName = Path.GetFileName(scope).ToLower();
                if (IsExactMatch(rootName, patterns) && !Directory.EnumerateFileSystemEntries(scope).Any())
                    ForceDeleteDirectory(scope);
            }
            catch { }
        }

        // Hard delete explicit leftover folders from the report.
        foreach (var hardFolder in opencodeHardDeleteFolders)
        {
            try
            {
                UpdateProgress("OPENCODE DIR: " + hardFolder, true);
                if (Directory.Exists(hardFolder))
                {
                    Console.WriteLine("  [OPENCODE HARD DIR] " + hardFolder);
                    ForceDeleteDirectory(hardFolder);
                }
            }
            catch { }
        }
